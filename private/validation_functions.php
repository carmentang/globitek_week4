<?php
  ...
  // has_valid_password_format('bijouisthebest')
  function has_valid_password_format($value) {
    $p1 = '/[A-Z]/';
    $p2 = '/[a-z]/';
    $p3 = '/[!@#$%^&*()\-_=+{};:,<.>]/';
    $p4 = '/[0-9]/';
    if(preg_match_all($p1,$value, $o) < 1) return false;
    if(preg_match_all($p2,$value, $o) < 1) return false;
    if(preg_match_all($p3,$value, $o) < 1) return false;
    if(preg_match_all($p4,$value, $o) < 1) return false;
    return true;
  }
?>
