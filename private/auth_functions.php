<?php
  ...
  function failed_login($username) {
    ...
    $result = find_failed_users_by_username($username);
    $failed_user = db_fetch_assoc($result);
    if(isset($failed_user)) {
      if($failed_user['count'] >= $threshold) {
        $last_attempt = strtotime($failed_user['last_attempt']);
      ...
      $failed_user['last_attempt'] = date("Y-m-d H:i:s");
      update_failed_attempt($failed_user);
    } else {
      $failed_user['username'] = $username;
      $failed_user['last_attempt'] = date("Y-m-d H:i:s");
      insert_failed_attempt($failed_user);
    }
  }

  function generate_strong_password($length=12) {
    $arr = array_merge(range('A', 'Z'), range('a', 'z'), range(0, 9), str_split('`~!@#$%^&*()-_=+]}[{;:,<.>/?"\|'));
    $str = implode($arr);
