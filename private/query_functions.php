<?php
  ...
  function validate_old_user($user, $errors=array()) {
    ...
    if (is_blank($user['previous_password'])) {
      $errors[] = "Please enter your previous password.";
    } elseif (!my_password_verify($user['previous_password'], $user['hashed_password'])) {
  ...
  function validate_new_user($user, $errors=array()) {
    if (is_blank($user['first_name'])) {
      $errors[] = "First name cannot be blank.";
    } elseif (!has_length($user['first_name'], array('min' => 2, 'max' => 255))) {
      $errors[] = "First name must be between 2 and 255 characters.";
  ...
  function insert_user($user) {
    ...
    $hashed_password = my_password_hash($user['password']);
  ...
  function insert_failed_attempt($failed_user) {
    global $db;
    $sql = "INSERT INTO failed_logins " ;
    $sql .= "(username, count, last_attempt) ";
    $sql .= "VALUES (";
    $sql .= "'" . db_escape($db, $failed_user['username']) . "',";
    $sql .= "1,";
    $sql .= "'" . $failed_user['last_attempt'] . "'";
    $sql .= ");";
  ...
  function update_failed_attempt($failed_user) {
    global $db;

    $count = $failed_user['count'] + 1;
    $sql = "UPDATE failed_logins SET ";
    $sql .= "count='" . $count . "', ";
    $sql .= "last_attempt='" . $failed_user['last_attempt'] . "' ";
    $sql .= "WHERE username='" . db_escape($db, $failed_user['username']) . "' ";
    $sql .= "LIMIT 1;";
  ...
  // find_failed_users_by_username('rockclimber67');
  function find_failed_users_by_username($username='') {
    global $db;
  ...
  // Update failed attempts
  function reset_count($failed_user) {
    global $db;
